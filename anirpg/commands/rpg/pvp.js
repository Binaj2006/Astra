...
async function sendTurnPrompt(sock, chatId, p1, p2, p1Id, p2Id, p1Rank, p2Rank) {
  ...
  txt+=`/<classcmd> [skill]       — Use a skill\n`;
...
async function handleVictory(sock, chatId, winner, loser, wId, lId, db, save, wStats, lStats, totalTurns) {
  ...
  // ── Daily Quest progress (NEW) — tracks pvp type, awards if hit ──
  // Fire-and-forget: notification goes via the post-command hook in the handler,
  // but we also call here to make the celebration appear in the same turn.
  try {
    const { trackAndNotify } = require('../../rpg/utils/QuestDispatcher');
    // Winner gets 'pvp' (a win) and the loser gets 'pvp' too (participated)
    trackAndNotify(winner, 'pvp', 1, sock, wId);
    trackAndNotify(loser,  'pvp', 1, sock, lId);
  } catch(e) {}
...
module.exports = {
  ...
  async execute(sock, msg, args, getDatabase, saveDatabase, sender) {
    ...
    // Check if PvP is enabled in this group (default: enabled)
    if (chatId.endsWith('@g.us')) {
      const gs = db.groupSettings?.[chatId];
      if (gs?.pvpEnabled === false) {
        return sock.sendMessage(chatId, {
          text: `⚔️ PvP is disabled in this group.\n\nAsk the owner to enable it with */set pvp --true*`,
        }, { quoted: msg });
      }
    }
    ...
    // ── IN-BATTLE ACTIONS ─────────────────────────────────────
    const battleActions=['attack','guard','taunt','feint','classcmd','special','ultimate','desperation','surrender'];
    ...
    if (action==='rematch') {
      ...
      if (player.stats.hp<player.stats.maxHp*0.3) return sock.sendMessage(chatId,{text:'❌ Need 30%+ HP! Use /use.'},{quoted:msg});
